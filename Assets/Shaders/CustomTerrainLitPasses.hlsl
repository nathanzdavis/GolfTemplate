...
float _MowStripeSpacing;
float _MowStripeWidth;
...
half GetMowStripeMask(float3 positionWS)
{
    ...
    float stripePosition = dot(positionWS.xz, direction);

    stripePosition /= max(_MowStripeSpacing, 0.001);

    float stripe = frac(stripePosition);

    float edge = _MowEdgeSoftness;
...
#endif
{
    ...
    // Get Fairway terrain-layer weight.
    // Fairway = Terrain Layer 0 = Red channel.
    half fairwayMask = splatControl.r;

    // Get mow stripe pattern.
        half mowMask = GetMowStripeMask(IN.positionWS);

    // Only apply mowing where Fairway exists.
        mowMask *= fairwayMask;

        half mowMultiplier = lerp(
        1.0h,
        1.0h - _MowDarkness,
        mowMask
    );
